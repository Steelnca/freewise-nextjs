...
import {
  SearchIcon, CalendarIcon, DollarSignIcon,
  SendIcon, FilterIcon,
} from 'lucide-react'
...
export default function FreelancerJobsPage() {
  ...
  const submitOffer = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail ?? 'Failed to submit offer'
      toast.error(msg)
    } finally {
      setSubmitting(false)
    }
    ...
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {loading ? '...' : `${allJobs.length} open jobs`}
        </p>
      </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${levelColors[job.experience_level]}`}>
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                      <span>by {job.client_username}</span>
                      ...
                      <span>{job.offer_count} {t.jobs.offers}</span>
                    </div>

                    {job.tags.length > 0 && (
                      <div className="flex gap-1 flex-wrap">
                        {job.tags.map(tag => (
                          <span key={tag.id} className="text-xs bg-muted px-2 py-0.5 rounded-full text-muted-foreground">
                            {tag.name}
                          </span>
                        ))}
                      </div>
      ...
      <Dialog open={!!offerJob} onOpenChange={open => !open && setOfferJob(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            {offerJob && (
              <p className="text-sm text-muted-foreground pt-1">{offerJob.title}</p>
            )}
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="cover_letter">{t.offers.coverLetter}</Label>
              <textarea
                id="cover_letter"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="proposed_price">{t.offers.proposedPrice}</Label>
                <Input
                  id="proposed_price"
                  type="number"
                  min={0}
                  value={offer.proposed_price}
                  onChange={e => setOffer(p => ({ ...p, proposed_price: e.target.value }))}
                  placeholder="15000"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="delivery_days">{t.offers.deliveryDays}</Label>
                <Input
                  id="delivery_days"
                  type="number"
                  min={1}
                  value={offer.delivery_days}
                  onChange={e => setOffer(p => ({ ...p, delivery_days: e.target.value }))}
                  placeholder="7"
                />
              </div>
            </div>
